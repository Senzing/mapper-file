...
class JSONReader(FileReader):
    ...
    def open(self):
        ...
        # If root is an object, look for common data array fields
            ...
            if (
                isinstance(self._data.get("meta"), dict)
                and isinstance(self._data["meta"].get("view"), dict)
                and "columns" in self._data["meta"]["view"]
                and "data" in self._data
            ):
...
class FileAnalyzer:
        ...
        if self.enumerate_attrs or self.is_pivot_enumeration:
            if self.is_pivot_enumeration:
                ...
                if group_by_attr:
                    self.pivot_stats = (
                        {}
                    )  # group_value -> {(attr1_val, attr2_val, ...): {value_attr_val: {count, records}}}
                else:
    ...
    def process_record(self, obj):
        ...
        if self.group_by_attr and isinstance(obj, dict):
            ...
            if group_value not in self.groups:
                self.groups[group_value] = {"nodes": {"root": Node("root")}, "record_count": 0}
                ...
                for value in values:
                    if value is not None and value != "":  # Skip None and empty string, but keep False
                        ...
                        if value_str not in self.enumeration_stats[group_value][attr_path]:
                            self.enumeration_stats[group_value][attr_path][value_str] = {"count": 0, "records": set()}
                ...
                for value in values:
                    if value is not None and value != "":  # Skip None and empty string, but keep False
                        ...
                        if value_str not in self.enumeration_stats[attr_path]:
                            self.enumeration_stats[attr_path][value_str] = {"count": 0, "records": set()}
    ...
    def process_pivot_enumeration(self, obj):
        ...
        for i in range(max_length):
            ...
            if value_str not in group_pivot_stats[grouping_key]:
                group_pivot_stats[grouping_key][value_str] = {"count": 0, "records": set()}
            ...
            for i in range(len(path_parts) - 1, 0, -1):
                ...
                if ancestor_path in list_nodes:
                    ...
                    for child_path, child_node in self.nodes.items():
                        if (
                            child_path.startswith(ancestor_path + ".")
                            and "." not in child_path[len(ancestor_path) + 1 :]
                        ):
    ...
    def detect_table_type(self, table_name=None):
        ...
        for node_key, node in self.nodes.items():
            ...
            if field_name.endswith("_id") or field_name.startswith(":@") or field_name.startswith(":id"):
                id_field_count += 1
    ...
    def detect_code_lists(
        self, max_unique_values=100, min_unique_count=2, max_unique_pct=0.10, min_population_pct=0.05
    ):
        ...
        for node_path, node in self.nodes.items():
            ...
            table_count = (
                node.table_record_count
                if hasattr(node, "table_record_count") and node.table_record_count > 0
                else node.record_count
            )
    ...
    def generate(self, template):
        ...
        Args:
            template: Type of report ('report', 'markdown', 'enumeration', 'code')
        ...
        if template == "report":
            ...
            reporter = get_reporter("code", self)
        else:
    ...
    def _generate_grouped(self):
        ...
        for group_value in sorted(self.analyzer.groups.keys()):
            ...
            for node in self._traverse_nodes(group_nodes, group_nodes["root"]):
                ...
                rows.append(
                    [group_value, attr_code, attr_type, record_cnt, record_pct, unique_cnt, unique_pct] + top_values
                )
        ...
        for node in self._traverse_nodes(self.analyzer.nodes, self.analyzer.root_node):
            ...
            if field_type in ("dict", "list"):
                ...
                table_count = (
                    node.table_record_count
                    if hasattr(node, "table_record_count") and node.table_record_count > 0
                    else self.analyzer.record_count
                )
            ...
            row_parts = [
                f"| {row_num} | {field_name} | {field_type} | {record_cnt} | {pop_pct} | {unique_pct} | {table_context_name} |"
            ]
    ...
    def _generate_standard_enumeration(self):
        ...
        for attr_path in sorted(self.analyzer.enumerate_attrs):
            ...
            for code_value, stats in sorted_codes:
                ...
                unique_pct = (
                    round(unique_records / self.analyzer.record_count * 100, 2) if self.analyzer.record_count else 0
                )
                ...
                rows.append(
                    [attr_path, code_value, record_cnt, record_pct, unique_records, unique_pct] + sample_records
                )
        ...
        for group_value in sorted(self.analyzer.enumeration_stats.keys()):
            ...
            for attr_path in sorted(self.analyzer.enumerate_attrs):
                ...
                for code_value, stats in sorted_codes:
                    ...
                    rows.append(
                        [group_value, attr_path, code_value, record_cnt, record_pct, unique_records, unique_pct]
                        + sample_records
                    )
    ...
    def _generate_pivot_enumeration(self):
        ...
        if self.analyzer.group_by_attr:
            ...
            for group_value in self.analyzer.pivot_stats.keys():
                ...
                group_record_count = (
                    self.analyzer.groups[group_value]["record_count"]
                    if self.analyzer.groups
                    else self.analyzer.record_count
                )
            ...
            for grouping_key, value_stats in sorted(self.analyzer.pivot_stats.items()):
                ...
                record_pct = (
                    round(total_record_cnt / self.analyzer.record_count * 100, 2) if self.analyzer.record_count else 0
                )
                unique_values = len(value_stats)
                unique_pct = round(unique_values / total_record_cnt * 100, 2) if total_record_cnt else 0
...
class CodeReporter(BaseReporter):
    """Generates Python mapper code from schema analysis."""

    def generate(self):
        """Generate Python mapping code rows."""
        if self.analyzer.group_by_attr and self.analyzer.groups:
            return self._generate_grouped()
        return self._generate_standard()
    ...
    def _generate_grouped(self):
        """Generate Python mapping code for grouped analysis.

        Merges all unique attributes across groups and adds comments
        showing which groups have each attribute.
        """
        rows = []
        merged_nodes = {}  # attr_key -> {node, groups, total_record_count, unique_values}
        group_names = sorted(self.analyzer.groups.keys())

        # Add header showing grouping info
        rows.append(f"# Grouped by: {self.analyzer.group_by_attr}")
        rows.append(f"# Groups found: {', '.join(group_names)}")
        rows.append("#")

        # Build merged node info with group membership tracking
        for group_name in group_names:
            ...
            for node in self._traverse_nodes(self.analyzer.groups[group_name]["nodes"], group_root):
                ...
                merged_nodes[node.node_desc]["groups"].append(group_name)
                merged_nodes[node.node_desc]["total_record_count"] += node.record_count
                for val, cnt in node.unique_values.items():
                    merged_nodes[node.node_desc]["unique_values"][val] = (
                        merged_nodes[node.node_desc]["unique_values"].get(val, 0) + cnt
                    )

        # Generate code using first group's node order, then any remaining
        processed_keys = set()
        self._emit_merged_nodes(rows, merged_nodes, group_names, processed_keys)
        return rows

    def _emit_merged_nodes(self, rows, merged_nodes, group_names, processed_keys):
        """Emit code for merged nodes in consistent order."""
        if group_names:
        ...
        for attr_key in sorted(merged_nodes.keys()):
            if attr_key not in processed_keys:
                self._emit_node(rows, merged_nodes[attr_key], group_names)

    def _emit_node(self, rows, info, group_names):
        """Emit code for a single merged node."""
        group_info = {"unique_values": info["unique_values"], "groups": info["groups"], "all_groups": group_names}
        rows.extend(self._generate_node_code(info["node"], info["total_record_count"], group_info))

    def _generate_node_code(self, node, total_record_count, group_info=None):
        """Generate code rows for a single node."""
        unique_values = group_info["unique_values"] if group_info else node.unique_values
        record_cnt = total_record_count if group_info else node.record_count
        base_count = self.analyzer.record_count if group_info else total_record_count

        depth = len(node.node_desc.split("."))
        indent = "" if depth == 1 else "    " * (depth - 1)
        prior = "raw_data" if depth == 1 else f"raw_data{depth - 1}"
        attr = node.node_desc.rsplit(".", 1)[-1]

        rows = ["", f"{indent}# attribute: {node.node_desc} ({node.node_type})"]

        if group_info:
            if set(group_info["groups"]) == set(group_info["all_groups"]):
                rows.append(f"{indent}# groups: ALL")
            else:
                rows.append(f"{indent}# groups: {', '.join(sorted(group_info['groups']))}")

        rows.append(
            f"{indent}# {round(record_cnt / base_count * 100, 2) if base_count else 0}% populated, "
            f"{round(len(unique_values) / record_cnt * 100, 2) if record_cnt else 0}% unique"
        )
        for k, v in sorted(unique_values.items(), key=lambda x: x[1], reverse=True)[:5]:
            rows.append(f"{indent}#      {str(k)[0:50]} ({v})")

        if node.node_type in ("list", "np.ndarray"):
            rows.append(f'{indent}for raw_data{depth} in ({prior}.get("{attr}") or []):')
        elif node.node_type == "dict":
            rows.append(f'{indent}if {prior}.get("{attr}"):')
            rows.append(f'{indent}    raw_data{depth} = {prior}.get("{attr}")')
        else:
            rows.append(f'{indent}# json_data.add_feature({{"{attr}": {prior}.get("{attr}")}})')
...
def create_python_script(code_rows, file_type, encoding):
    """Create a Python mapper script from generated code rows.

    Args:
        code_rows: List of code lines from CodeReporter.generate()
        file_type: Type of input file (csv, json, jsonl, parquet, xml)
        encoding: File encoding
        ...
        for line in file:
            ...
            if line.startswith(import_here):
                if file_type == "csv":
                    ...
                    script_rows.append("import xml.etree.ElementTree as ET")
                ...
                if file_type == "parquet":
                    script_rows.append(indent + 'file = pd.read_parquet(file_name, engine="auto")')
                ...
                if file_type not in ("parquet", "xml"):
...
GROUP_BY FORMATS:
  ...
  Filtered:  --group_by schema=Person        (group by schema, show only Person records)
    ...
    parser.add_argument("input_file", help="Input file or directory path (supports CSV, JSON, JSONL, Parquet, XML)")
    parser.add_argument(
        "-t", "--file_type", help='File type: "csv", "jsonl", "json", "parquet", "xml" (auto-detected if not specified)'
    )
    parser.add_argument("-e", "--encoding", default="utf-8", help="File encoding (default: utf-8)")
    parser.add_argument(
        "-o", "--output_file", help="Output CSV file path (required for --enumerate, optional for schema analysis)"
    )
    parser.add_argument(
        "--top_values", type=int, default=5, help="Number of top values to display/analyze (default: 5)"
    )
    parser.add_argument("--filter", help="Filter records: 'attribute=value' (e.g., 'status=active', 'type=Person')")
    parser.add_argument(
        "--group_by",
...
Example: 'properties:type,country:number'""",
    ...
    parser.add_argument("-p", "--python_file_name", help="Generate Python mapper script from schema analysis")
                    ...
                    if isinstance(row, ParseError):
                        ...
                        if total_processed >= MIN_RECORDS_FOR_THRESHOLD:
                            ...
                            if error_pct >= ERROR_THRESHOLD_PCT:
                                print(
                                    f"\nABORTED: Error rate {error_pct:.1f}% exceeds {ERROR_THRESHOLD_PCT}% threshold",
                                    file=sys.stderr,
                                )
                                shut_down = 1
                                break
    ...
    if enumerate_config:
            ...
            if len(enum_report) > 1:  # Has data beyond header
                if args.output_file:
                    ...
                    if prettytable:
                        ...
                        print(
                            f"{'Attribute':<30} {'Code Value':<20} {'Count':<8} {'Pct':<8} {'Records':<8} {'Sample Records':<50}"
                        )
                        print("-" * 120)
                        for row in enum_report[1:]:
                            if len(row) >= 6:
                                ...
                                print(
                                    f"{row[0]:<30} {row[1]:<20} {row[2]:<8} {row[3]:<8} {row[4]:<8} {sample_records:<50}"
                                )
                        print()
            else:
        ...
        if args.output_file:
            ...
            if header:
                output_lines.append(
                    f"{'Attribute':<25} {'Type':<15} {'Count':<8} {'Pct':<8} {'Unique':<8} {'Top Value':<30}"
                )
                output_lines.append("-" * 100)
                for row in report_rows[1:]:  # Skip header row
                    if len(row) >= 7:  # Ensure we have enough columns
                        output_lines.append(
                            f"{row[0]:<25} {row[1]:<15} {row[2]:<8} {row[3]:<8} {row[4]:<8} {row[6]:<30}"
                        )
            output_lines.append("")
    ...
    # Generate Python mapper script if requested
